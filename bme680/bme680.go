...
func (d *Dev) readDataRegisters(e *physic.Env) error {
	...
	var tempComp, pressureComp, humidityComp float32

	tempComp = d.calibration.compensateTemp(tRaw)
	tempCompNanoKelvin := tempComp*float32(physic.Celsius) + float32(physic.ZeroCelsius)
	e.Temperature = physic.Temperature(tempCompNanoKelvin)

	if d.opts.Pressure != Off {
		pressureComp = d.calibration.compensatePressure(pRaw)
		pressureCompNanoPascal := pressureComp * float32(physic.Pascal)
		e.Pressure = physic.Pressure(pressureCompNanoPascal)
	}

	if d.opts.Humidity != Off {
		humidityComp = d.calibration.compensateHumidity(hRaw)
		humidityCompRH := humidityComp * float32(physic.PercentRH)
